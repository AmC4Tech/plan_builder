...
export const prompts: Record<string, (ctx: PromptContext) => string> = {
    ...
    // DOCX: 09实施/22培训方案/系统培训方案模板.doc
    "09实施/22培训方案/系统培训方案模板.doc": (ctx) => `
你是一个专业的项目文档编写助手。
...
`,
    ...
    // DOCX: 09实施/22培训方案/系统培训方案模板.docx
    "09实施/22培训方案/系统培训方案模板.docx": (ctx) => `
你是一个专业的项目文档编写助手。
请根据以下章节标题列表，结合项目背景"${ctx.projectData.projectDescription}"，为项目"${ctx.projectData.projectName}"编写对应的内容。
请返回一个 JSON 对象，Key 是章节标题（必须完全匹配），Value 是对应的内容。
...
`,
    ...
    // DOCX: 09实施/23培训记录/2.培训签到表.doc
    "09实施/23培训记录/2.培训签到表.doc": (ctx) => `
你是一个专业的项目文档编写助手。
请根据以下章节标题列表，结合项目背景"${ctx.projectData.projectDescription}"，为项目"${ctx.projectData.projectName}"编写对应的内容。
请返回一个 JSON 对象，Key 是章节标题（必须完全匹配），Value 是对应的内容。
...
`,
    ...
    // DOCX: 09实施/23培训记录/2.培训签到表.docx
    "09实施/23培训记录/2.培训签到表.docx": (ctx) => `
你是一个专业的项目文档编写助手。
请根据以下章节标题列表，结合项目背景"${ctx.projectData.projectDescription}"，为项目"${ctx.projectData.projectName}"编写对应的内容。
请返回一个 JSON 对象，Key 是章节标题（必须完全匹配），Value 是对应的内容。
